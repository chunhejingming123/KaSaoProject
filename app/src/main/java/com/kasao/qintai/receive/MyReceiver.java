package com.kasao.qintai.receive;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            try {
                ...
                if (url.startsWith("http://") || url.startsWith("https://")) { //http
                    title = jsonObject.getString("title");
                    Intent intentsplash = new Intent(context, SplashActivity.class);
                    intentsplash.putExtra(FrameParmars.KEY_URL, url);
                    intentsplash.putExtra(FrameParmars.KEY_TITLE, title);
                    intentsplash.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    Intent i = new Intent(context, WebActivity.class);
                    i.putExtra(FrameParmars.KEY_URL, url);
                    i.putExtra(FrameParmars.KEY_TITLE, title);
                    i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    Intent[] intents = {intentsplash, i};
                    context.startActivities(intents);
                } else {
                    LogUtil.e(TAG, "-----else =url" + url);
                    ForwardUtil.startUri(url, context);
                }
            } catch (Throwable throwable) {
                LogUtil.e(TAG, "-----throwable =" + throwable.getLocalizedMessage());
