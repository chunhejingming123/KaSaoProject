...
public class DropDownMenu extends RelativeLayout {
    ...
    public DropDownMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        LayoutParams lp = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dpTpPx(0.8f));
        lp.addRule(RelativeLayout.BELOW, R.id.dropDownMenu_tabMenuView);
        underLine.setLayoutParams(lp);
        underLine.setBackgroundColor(underlineColor);
        addView(underLine, 1);
        ColorDrawable dw = new ColorDrawable(0xb0000000);
        setBackgroundDrawable(dw);//getResources().getDrawable(R.drawable.shape_top_round_red)
    }
...
//                null, null, null);
        ...
        if (i < tabTexts.size() - 1) {
            ...
            ViewGroup.LayoutParams lp = new LayoutParams(dpTpPx(1.0f), dpTpPx(15f));//ViewGroup.LayoutParams.MATCH_PARENT
    ...
    public void openMenu(int current_tab_position) {
        if (current_tab_position != -1) {
            TextView tab = getTabMenuTextView(current_tab_position);

            tab.setCompoundDrawablesWithIntrinsicBounds(null, null,
                    getResources().getDrawable(menuUnselectedIcon), null);
            switchMenu(tab, current_tab_position);
        }
    ...
    private void switchMenu(View target, int index) {
        if (null != mChoseBanner) {
            mChoseBanner.onChoseItem(index);
        }
    ...
    private IChoseBanner mChoseBanner;

    public void setChoseBanner(IChoseBanner mchoseBanner) {
        mChoseBanner = mchoseBanner;
    }

    public interface IChoseBanner {
        void onChoseItem(int index);
    }
}
