...
import static com.kasao.qintai.util.DateUtils.dateToString;
import static com.kasao.qintai.util.DateUtils.stringToDate;
...
public class DateUtil {
    ...
    public static String getLongPointDate(long lo, String format) {
        Date date = new Date(lo);
    ...
    public static String getDataNoPoint(long lo) {
        Date date = new Date(lo);
        SimpleDateFormat sd = new SimpleDateFormat(YEARMONTHDAY);
        return sd.format(date);
    }

    public static String getYearMonth(long lo) {
        Date date = new Date(lo);
        SimpleDateFormat sd = new SimpleDateFormat(YEAR_MONTH);
        return sd.format(date);
    }
    ...
    public static String getJiDuTime(long time) {
    ...
    public static String[] get14DayTime() {
        String[] time14Day = new String[15];
        for (int i = 0; i < 15; i++) {
            Date nowDate = new Date();
            Calendar calendar = Calendar.getInstance();
            //   calendar.setTimeInMillis(System.currentTimeMillis());//-DateUtil.DAY_MILLISECOND
            calendar.setTime(nowDate);
...
//        calendarZero.set(Calendar.MILLISECOND, 0);
     ...
     * 获取今天之前的30天
     */
    public static List<String> get30day2tofayList() {
        List<String> months = new ArrayList<>();
        Calendar c = Calendar.getInstance();
        long current_time = System.currentTimeMillis();
        long day_ms = 24 * 60 * 60 * 1000;
    ...
    public static String getCountDownHSM(long countDownTime) {
        String timeStr = "00天00时00分00秒";
        if (countDownTime > 0) {
            long day = countDownTime / DateUtil.DAY_MILLISECOND;
            long hours = (countDownTime - day * DateUtil.DAY_MILLISECOND) / (1000 * 60 * 60);
            long minutes = (countDownTime - day * DateUtil.DAY_MILLISECOND - hours * (1000 * 60 * 60)) / (1000 * 60);
            long second = (countDownTime - day * DateUtil.DAY_MILLISECOND - hours * (1000 * 60 * 60) - minutes * (1000 * 60)) / 1000;
            String daystr = day > 9 ? day + "" : "0" + day;
            String hoursStr = hours > 9 ? hours + "" : "0" + hours;
            String minutesStr = minutes > 9 ? minutes + "" : "0" + minutes;
            String secondStr = second > 9 ? second + "" : "0" + second;
            timeStr = daystr + "天" + hoursStr + "时" + minutesStr + "分" + secondStr + "秒";
        }
        return timeStr;
    }
     ...
     * @return
     */
    public static String getBeforMonth(int index) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        Calendar c = Calendar.getInstance();
        c.setTime(new Date());
        c.add(Calendar.MONTH, -index);
        Date m = c.getTime();
        return formatYearsMonthDay(m.getTime());
    }
     ...
     * @return
     */
    public static String getBeforQuarter(int index) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        Calendar c = Calendar.getInstance();
        c.setTime(new Date());
        c.add(Calendar.MONTH, -3 * index);
        Date m = c.getTime();
        return formatYearsMonthDay(m.getTime());
    }
     ...
     * @return
     */
    public static String getBeforeYear(int index) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        Calendar c = Calendar.getInstance();
        c.setTime(new Date());
    ...
    public static String getMonthDay(long time) {
        ...
        SimpleDateFormat sd = new SimpleDateFormat(HOUR_MINUTE2);
        return sd.format(date).substring(3);
    }

    public static String getMonthDay4(long time) {
        Date date = new Date(time);
        SimpleDateFormat sd = new SimpleDateFormat(HOUR_MINUTE2);
        return sd.format(date);
    }

    // 根据类型返回时间段  //currentIime - (PageIndex + 1) * DateUtil.DAY_MILLISECOND,
    public static String getTimeTag(String timeType, long data, int index) {
        switch (timeType) {
            case "1":
                return getLongPointDate(data) + "";
            case "3":
                return getYearMonth(data - 30 * index * DAY_MILLISECOND) + "";
            case "4":
                return getJiDuTime(data) + "";
            case "5":
                return getYear(data) + "";
        }
        return "";
